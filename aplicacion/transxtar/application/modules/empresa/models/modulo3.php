...
class Modulo3 extends CI_Model {
    ...
    function obtenerModulo($nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo){
    	$modulo3 = array();
    	$sql = "SELECT C.modulo3, IO.inalo, IO.inalovd, IO.inaloatc, IO.inali, IO.inba, IO.insr, IO.inoe, IO.inoio, IO.intio
                FROM rmmh_form_ingoperacionales IO, rmmh_admin_control C
                WHERE IO.nro_orden = C.nro_orden
                AND IO.nro_unidad_local = C.nro_unidad_local
    	...
    	if ($query->num_rows() > 0){
    		foreach ($query->result() as $row){
    			...
    			$modulo3["inalovd"] = $row->inalovd;
    			$modulo3["inaloatc"] = $row->inaloatc;
    ...
    function actualizarModulo($inalo, $inalovd, $inaloatc, $inali, $inba, $insr, $inoe, $inoio, $intio, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo){
    ...
    function insertarModulo($inalo, $inalovd, $inaloatc, $inali, $inba, $insr, $inoe, $inoio, $intio, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo){
    	$data = array('inalo' => $inalo,
    			      'inalovd' => $inalovd,
    			      'inaloatc' => $inaloatc,
    	              'inali' => $inali,
