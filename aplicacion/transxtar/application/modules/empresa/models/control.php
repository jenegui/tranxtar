...
class Control extends CI_Model {
    ...
    function obtenerNovedadEstado(){
    	$novestado = array('novedad' => 0, 'estado' => 0);
    	$nro_orden = $this->session->userdata('nro_orden');      				//Obtener desde la sesion     
		$nro_establecimiento = $this->session->userdata('unidad_local'); //Obtener desde la sesion  
    ...
    /**
     * Obtiene unidad local de la tabla control.
     * @author sjneirag
     * @since  12/05/2017
     */
    function obtenerUlocalControl($nro_orden,$nro_unidad_local,$ano_periodo,$mes_periodo){
    	$ulocal = array();
    	$sql = "SELECT nro_orden, nro_unidad_local, ano_periodo, mes_periodo, estado_control
    	FROM rmmh_admin_control
    	WHERE
    	nro_orden=$nro_orden
    	AND nro_establecimiento= $nro_unidad_local
    	AND ano_periodo=$ano_periodo
    	AND mes_periodo=$mes_periodo
    	AND estado_control=1 ";
    	$query = $this->db->query($sql);
		if ($query->num_rows() > 0){
    		foreach ($query->result() as $row){
    			$ulocal["nro_orden"] = $row->nro_orden;
    			$ulocal["nro_unidad_local"] = $row->nro_unidad_local;
    		}
    	}else{
    		$ulocal["nro_orden"] = 0;
    		$ulocal["nro_unidad_local"] = 0;
    	}
    	$this->db->close();
    	return $ulocal;
    }
    ...
    /**
     * Inserta las unidades locales en la tabla control
     * @author sjneirag
     * @since  08/05/2017
     */
    function insertarControl($nro_orden,$nro_establecimiento,$ano_periodo, $mes_periodo, $fk_sede, $fk_subsede){
    	$data = array(
    			'nro_orden'=>$nro_orden,
