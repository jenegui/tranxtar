...
class Usuario extends CI_Model {
	...
	function contarFuentes($uno, $dos){
		$total = 0;
   ...
   //Obtiene todos los usuarios del sistema que son fuentes (Filtrando para un a�o y un periodo)
    ...
    function obtenerDestinatarios() {
        $destinatarios = array();
        $sql = "SELECT id_destinatario, nro_identificacion, concat_ws(' - ', nombre_destinatario, direccion_destinatario, nom_mpio) as destinatario,
    ...
    //Obtiene las ciudes por destinario
    function obtenerCiudadesDest($nro_identificacion) {
        $ciudadDestinatario = array();
        $sql = "SELECT id_destinatario, id_mpio, concat_ws(' - ',nom_mpio, direccion_destinatario) as nomMpio
            FROM  txtar_admin_destinatarios, txtar_param_mpios
            WHERE ciudad_destinatario=id_mpio
        ...
        if ($query->num_rows() > 0) {
            $i = 0;
            foreach ($query->result() as $row) {
                $ciudadDestinatario[$i]["id_destinatario"] = $row->id_destinatario;
                $ciudadDestinatario[$i]["codigo"] = $row->id_mpio;
                $ciudadDestinatario[$i]["nombre"] = $row->nomMpio;
    ...
    function obtenerOperariosInternos() {
        $operarios = array();
        $sql = "SELECT id_usuario, num_identificacion, nom_usuario      
            FROM  txtar_admin_usuarios
            WHERE fk_rol=5
        ...
        if ($query->num_rows() > 0) {
            $i = 0;
            foreach ($query->result() as $row) {
                $operarios[$i]["id_usuario"] = $row->id_usuario;
