...
class Empresa extends MX_Controller {
	...
	public function actualizarModuloI(){
		...
		$this->modulo1->actualizarModulo($nro_orden, $nro_establecimiento, $idnit, $idproraz, $iddirecc, $iddepto, $idmpio, $idtelno, $idfaxno, $idpagweb, $idcorreo, $finicial, $ffinal);
		...
		$cuenta= count($data["unidadesLocales"]);
		for($i=0; $i<=$cuenta-1; $i++){
			$nro_unidad_local=$nro_orden.$_REQUEST['codmpio'.$i];
			$fk_sede=$data["unidadesLocales"][$i]["sede"];
			$fk_subsede=$data["unidadesLocales"][$i]["subsede"];
			$usuario = $this->session->userdata("id");
			
			//Busca que no exista en la tabla control 
			$data["control"] = $this->control->obtenerUlocalControl($nro_orden,$nro_unidad_local,$ano_periodo,$mes_periodo);
			
			if($data["control"]["nro_unidad_local"]==$nro_unidad_local){
				echo "La unidad local número ".$nro_unidad_local. " Ya se encuentra registrada en la tabla control";
			}else{
				//Inserta en la tabla control las unidades locales
				$this->control->insertarControl($nro_orden,$nro_unidad_local,$ano_periodo, $mes_periodo, $fk_sede, $fk_subsede);
			}
		}*/
	 ...
	 * @since  08/05/2017
	 */
	public function actualizarModuloEst(){
		$this->load->model("control");
		$this->load->model("modulo1");
		$ano_periodo = $this->session->userdata("ano_periodo");
		$mes_periodo = $this->session->userdata("mes_periodo");
