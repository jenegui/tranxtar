<?php
if(!isset($datos["nro_establecimiento"]) || $datos["nro_establecimiento"]==''){
    echo "<legend style='color:red;'>La fuente no ha sido asignada a un critico.</legend>";  
    EXIT;
}     
?>
<div id="content">
		  ...
		  <td><select id="cmbNovedad" name="cmbNovedad" class="select" <?php if ($bloqueo){ echo 'disabled="disabled"'; } ?><?php if ($rol==6){ echo 'disabled="disabled"'; }?>>
		        ...
		        <?php for ($i=0; $i<count($novedades); $i++){
		        	  		if ($novedades[$i]["id"]==$novedad["fk_novedad"]){ 
		        	  			echo '<option value="'.$novedades[$i]["id"].'" selected="selected">'.utf8_decode($novedades[$i]["nombre"]).'</option>';
		        	  		}	   
		        			else{
		        				echo '<option value="'.$novedades[$i]["id"].'">'.utf8_decode($novedades[$i]["nombre"]).'</option>';
		        			}
		              }
		        ?>
		      </select>
