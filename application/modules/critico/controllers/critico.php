...
class Critico extends MX_Controller {
	...
	//Funciones AJAX Utilizadas en el modulo de cr�tica
	//*************************************************
	...
	public function notificar($empresa, $establecimiento){
            $this->load->model("periodo");
            $this->load->model("control");
            $nom_usuario = $this->session->userdata("nombre");
            $tipo_usuario = $this->session->userdata("tipo_usuario");
            ...
            $data["menu"] = "criticomenu";
            $data["periodos"] = $this->periodo->obtenerPeriodosTodosMOD();
            $data["notificacion"]=$this->control->obtenerNotificacion($empresa, $establecimiento, $ano_periodo, $mes_periodo);
            $this->load->view("layout",$data);
        }
        ...
        public function guardarNotificacion(){
            ...
            $idcritico=$nom_usuario = $this->session->userdata("id");
            $data["nom_usuario"] = $nom_usuario;
            $data["controller"] = "critico";
            $data["menu"] = "criticomenu";
            $ano_periodo = $this->session->userdata("ano_periodo");
            $mes_periodo = $this->session->userdata("mes_periodo");
            $fechaNot=$this->input->post("fechaNotificacion");
            $fechaNotificacion=$this->general->formatoFecha($fechaNot,'/');
            $orden=$this->input->post("nro_orden");
            $establecimiento=$this->input->post("nro_establecimiento");
            $this->control->guardarNotificacion($orden, $establecimiento, $fechaNotificacion, $ano_periodo, $mes_periodo, $idcritico);
        }    
	...
	// 4) Guarda las modificaciones en cr�tica que se hayan realizado al capitulo I del formulario
		...
		switch($capitulo){
			case 1:  //Guardar los datos del formulario.
				     $this->modulo1->actualizarModulo($nro_orden, $nro_establecimiento, $idnit, $idproraz, $idnomcom, $idsigla, $iddirecc, $iddepto, $idmpio, $idtelno, $idfaxno, $idpagweb, $idcorreo, $finicial, $ffinal,
                            ...
                            $this->observacion->actualizarObservacion($nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo, 1, 'idproraz', $idproraz, $obsidproraz);
			...
			case 3:  //Guardar los datos del formulario.
					...
					if (!isset($obsintio1)){
					 ...
					 $this->observacion->actualizarObservacion($nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo, 3, 'incat', $incat, $obsincat);*/
					 $this->observacion->guardarObservacion($capitulo, $campo, $observacion, $fecha, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo, $idcritico);
			         break;
			case 4:  //Guardar los datos del formulario.
					...
					if(!isset($obsmvamnr)){
					 ...
					 $this->observacion->actualizarObservacion($nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo, 4, 'thumult', $thumult, $obsthumult);
                                         $this->observacion->actualizarObservacion($nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo, 4, 'mvam', $mvam, $obsmvam);
					 $this->observacion->actualizarObservacion($nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo, 4, 'mvamnr', $mvamnr, $obsmvamnr);*/
					 $this->observacion->guardarObservacion($capitulo, $campo, $observacion, $fecha, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo, $idcritico);
			         break;
