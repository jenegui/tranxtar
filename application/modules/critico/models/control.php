...
class Control extends CI_Model {
	...
	function guardarNotificacion($orden, $establecimiento, $fechaNotificacion, $ano_periodo, $mes_periodo, $idcritico){
                $estado=1;
   		$data = array('fecha_notificacion' => $fechaNotificacion,
		              'fk_estado' => $estado  
		        );
		$this->db->where("nro_orden", $orden);
		$this->db->where("nro_establecimiento", $establecimiento);
		$this->db->where("ano_periodo", $ano_periodo);
		$this->db->where("mes_periodo", $mes_periodo);
		$this->db->update("rmmh_admin_control",$data);
                $this->db->close();
        ...
        function obtenerNotificacion($orden, $establecimiento, $ano_periodo, $mes_periodo){
                $notificacion = array();
   		$sql = "SELECT C.nro_orden,
                    C.nro_establecimiento,
                    DATE_FORMAT(fecha_notificacion,'%d/%m/%Y') AS fecha_notificacion,
                    fk_estado,
                    nom_estado
                FROM rmmh_admin_control C, rmmh_admin_empresas EM, rmmh_admin_establecimientos ES, rmmh_param_estados EST
                ...
                AND C.ano_periodo = $ano_periodo
                AND C.mes_periodo = $mes_periodo
                AND C.nro_orden = $orden   
                AND C.nro_establecimiento= $establecimiento "; 
   		 
		$query = $this->db->query($sql);
		if ($query->num_rows()>0){
			$i=0;
			foreach($query->result() as $row){
			$notificacion["nro_orden"] = $row->nro_orden;
    			$notificacion["nro_establecimiento"] = $row->nro_establecimiento;
    			$notificacion["fecha_notificacion"] = $row->fecha_notificacion;
    			$notificacion["estado"] = $row->fk_estado;
                        $notificacion["nom_estado"] = $row->nom_estado;
    			$i++;
			}
		}
		$this->db->close();
