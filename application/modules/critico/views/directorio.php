	...
	<?php  
        for ($i=0; $i<count($fuentes); $i++){ 
            ...
            if($fuentes[$i]["tipo_encuesta"]=="M"){
                ...
                $base_url=base_url("miniencuesta/indexCritico/".$fuentes[$i]["nro_orden"]."/".$fuentes[$i]["nro_establecimiento"]);
            }elseif($fuentes[$i]["tipo_encuesta"]=="E"){
                $tipoEncuesta="Encuesta";
                $base_url=base_url("critico/detalleFormulario/".$fuentes[$i]["nro_orden"]."/".$fuentes[$i]["nro_establecimiento"]);
            }
        ?>
	<tr class="<?php echo $class; ?>">
  		<!-- td><a href="<?php //echo $base_url; ?>"><?php //echo $fuentes[$i]["nro_orden"]; ?></a></td-->
  		<td><a href="<?php echo $base_url ?>"><?php echo $fuentes[$i]["nro_orden"]; ?></a></td>
  		<td><a href="<?php echo $base_url ?>"><?php echo $fuentes[$i]["nro_establecimiento"]; ?></a></td>  		
  		...
  		<td class="redText"><?php echo $this->control->obtenerEstadoControl($fuentes[$i]["novedad"], $fuentes[$i]["estado"]); ?></td> 
                <td><?php echo $tipoEncuesta; ?></td>
                <td>
                    <a href="<?php echo site_url("/critico/notificar/".$fuentes[$i]["nro_orden"]."/".$fuentes[$i]["nro_establecimiento"]); ?>"><img src="<?php echo base_url("images/go.png"); ?>" border="0"/></a>
                </td>
