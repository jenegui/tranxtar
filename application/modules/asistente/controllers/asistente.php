...
class Asistente extends MX_Controller {
	...
	//Realiza la asignacion de fuentes a los criticos desde el modulo del asistente t�cnico (Para la misma sede del asistente tecnico)
	public function asignarFuentes($idcritico,$sede,$subsede){
		$this->load->model("periodo");
		$this->load->model("usuario");
               
		$idasistente = $this->session->userdata("id");
                $sede = $this->usuario->obtenerSede($idasistente);
                $subsede = $this->usuario->obtenerSubsede($idasistente);
                //$subsede=$this->session->userdata("subsede", $subsede);
		$ano_periodo = $this->session->userdata("ano_periodo");
		$mes_periodo = $this->session->userdata("mes_periodo");
		...
		$data["periodos"] = $this->periodo->obtenerPeriodosTodosMOD();
                $data["sedecritico"] = $sede;
		$data["subsedecritico"] = $subsede;
		$data["idcritico"] = $idcritico;
		$data["nomcritico"] = $this->usuario->obtenerNombreCritico($idcritico);
		$data["sinasignar"] = $this->usuario->obtenerFuentesSinAsignar($sede, $subsede, $ano_periodo, $mes_periodo);
	...
	public function asignarFuentesCritico(){
		...
		$usuario = $this->input->post("hddCritico");		 //Obtengo el ID del usuario
                ...
                $subsede = $this->input->post("subsede");
		...
		redirect("asistente/asignarFuentes/$usuario/$sede/$subsede","location", 301);
	}
	...
	public function removerFuentesCritico(){
		...
		redirect("asistente/asignarFuentes/$critico/$sede/$subsede","location", 301);
	}
	...
	// 4) Guarda las modificaciones en cr�tica que se hayan realizado a los capitulos del formulario 
	...
	// 9) Inserta un nuevo usuario "critico" desde el modulo de asistente t�cnico. 
