 ...
 * @since  Mayo 15 de 2017
 */


class Miniencuesta extends MX_Controller {
    ...
    /**
     * Pinta el formularion de la carátala No. 1.
     * @author sjneirag
     * @since  08/05/2017
     */
    public function index(){
        ...
        //$this->load->view("critico/criticomenu");
    	...
    	if($tipo_usuario==9){
            $data['tipo_usuario']="MINIENCUESTA";
        ...
        $data["envio"] = $this->envioform->obtenerModulo($nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo);  //ESTE HACE POR EL MODULO 4
        $index="miniencuesta";
        $data['observ'] = $this->observacion->consultarObservacion($index, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo);
        $data["numEst"] = $this->modulo1->obtenerNumEst($nro_orden,$ano_periodo,$mes_periodo,1);
    	$data["numEstApertMes"] = $this->modulo1->obtenerNumEst($nro_orden,$ano_periodo,$mes_periodo,2);
    	$data["numEstCierreMes"] = $this->modulo1->obtenerNumEst($nro_orden,$ano_periodo,$mes_periodo,3);
    	$data["unidadesLocales"] = $this->modulo1->obtenerUnidadLocal($nro_orden,$ano_periodo,$mes_periodo,3);
    	...
    	//$this->control->obtenerNovedadEstado($nro_establecimiento);
	//$data["bloqueo"] = $this->control->bloquearCampos($nro_establecimiento);
     ...
     * @since  08/05/2017
     */
    public function indexCritico($empresa, $establecimiento){
    	$this->load->model("periodo");
    	$this->load->model("divipola");
    	$this->load->model("control");
    	$this->load->model("modulo1");
    	...
    	$this->load->model("observacion");
        if(isset($empresa)){
           $this->session->set_userdata("nro_orden", $empresa);
           $nro_orden=$empresa;
        }else{
            $nro_orden = $this->session->userdata("nro_orden");
        }
        if(isset($establecimiento)){
            $this->session->set_userdata("nro_establecimiento", $establecimiento);
    	...
    	$data["periodos"] = $this->periodo->obtenerPeriodosPendientes($nro_orden, $nro_establecimiento);
    	$data["controller"] = "miniencuesta";
    	$data["novedad_estado"] = $this->control->obtenerNovedadEstado($empresa, $establecimiento, $ano_periodo, $mes_periodo);
    	//$data["view"] = "fuente";
    	$data["view"] = "modulo1";
    	$data["nom_usuario"] = $nom_usuario;
    	$data["nro_orden"] = $nro_orden;
    ...
    /**
     * Pinta el formularion para registrar nuevos estableciminetos. 1.
     * @author sjneirag
     * @since  08/05/2017
     */
    public function aper_establecimientos($nro_orden){
        $this->load->model("periodo");
    	$this->load->model("divipola");
    	$this->load->model("control");
    	$this->load->model("modulo1");
    	...
    	$tipo_usuario = $this->session->userdata("tipo_usuario");
    	if($tipo_usuario==8){
    		$data['tipo_usuario']="GERENCIA";
    	}
    	$data["periodos"] = $this->periodo->obtenerPeriodosPendientes($nro_orden, $nro_establecimiento);
    	$data["controller"] = "empresa";
    	$data["menu"] = "fuentemenu";
    	//$data["view"] = "fuente";
    	$data["view"] = "establecimientos";
    	$data["nom_usuario"] = $nom_usuario;
    	$data["nro_orden"] = $nro_orden;
    	$data["departamentos"] = $this->divipola->obtenerDepartamentos();
    	$data["municipios"] = $this->divipola->obtenerMunicipios(0); //Obtiene todos los municipios
    ...
    /**
     * Lista los establecimientos para seleccionar el que se va a marcar coo cerrados. 1.
     * @author sjneirag
     * @since  08/05/2017
     */
    public function cierre_establecimientos($nroEstab,$cierre){
    	$this->load->model("periodo");
    	$this->load->model("divipola");
    	$this->load->model("control");
    	$this->load->model("modulo1");
    	...
    	if($cierre=="ListaEst"){
    		$data["estabEmpresa"]=$this->modulo1->obtenerEstablecimientosXEmpresa($nro_orden,'',"empresa");
    		$data["cierre"]=$cierre;
    		$data["view"] = "listaEstablecimientos";
    	}elseif($cierre=="ListaEstUlocal"){
    		$data["estabEmpresa"]=$this->modulo1->obtenerEstablecimientosXEmpresa($nro_orden,$nroEstab,"ulocal");
    		$data["cierre"]=$cierre;
    		$data["view"] = "listaEstablecimientos";
    	}else{
    		$data["establec"]=$this->modulo1->obtenerEstablecimientosXEmpresa($nroEstab,'',"establecimiento");
    		$data["anio"]=$ano_periodo;
    		$data["mes"]=$mes_periodo;
    		$data["view"] = "cierraEstablecimientos";
    	}
    	$data["nom_usuario"] = $nom_usuario;
    	$data["nro_orden"] = $nro_orden;
    	$data["departamentos"] = $this->divipola->obtenerDepartamentos();
    ...
    /**
     * Función para los módulos 2,3,4.
     * @author sjneirag
     * @since  08/05/2017
     */
    public function modulos($nro_unidad_local){
    	$this->load->model("periodo");
    	$this->load->model("divipola");
    	$this->load->model("control");    	
    	$this->load->model("modulo1");  
    	...
    	$this->load->model("envioform");
    	$nro_orden = $this->session->userdata("nro_orden");
    	$nro_establecimiento = $nro_unidad_local;
    	//Registro la variable unidad local en la sesion 
    	$this->session->set_userdata("unidad_local", $nro_unidad_local);
    	...
    	$data["nom_usuario"] = $nom_usuario;
    	$data["nro_orden"] = $nro_orden;
    	$data["nro_establecimiento"] = $nro_unidad_local;
    ...
    //Rango para la variación anual y mensual para la ficha de análisis.
		...
		$this->control->obtenerNovedadEstado($nro_unidad_local);
		$data["bloqueo"] = $this->control->bloquearCampos($nro_unidad_local);
	...
	public function actualizarModuloI(){
		...
		$this->modulo1->actualizarModulo($nro_orden, $nro_establecimiento, $cambioNit, $idnit ,$iddv,$idnitNuevo, $iddvNuevo, $idproraz, $iddirecc, $iddepto, $idmpio, $idtelno, $idfaxno, $idpagweb, $idcorreo, $finicial, $ffinal);
                $data["modulo2"] = $this->modulo2->obtenerModulo($nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo);  //ESTE HACE POR EL MODULO 2
                $data["modulo3"] = $this->modulo3->obtenerModulo($nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo);  //ESTE HACE POR EL MODULO 3
                $data["modulo4"] = $this->modulo4->obtenerModulo($nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo);  //ESTE HACE POR EL MODULO 4
                
                 //Guarda módulo II
                if ($data["modulo2"]["op2"]=='insert'){
			//Insertar el registro en la tabla del modulo II
			$this->modulo2->insertarModulo($pottot, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo);
			//Actualizar el registro de la tabla de control
			
		}
                elseif($data["modulo2"]["op2"]=='update'){
			//Update sobre el registro de la tabla del modulo II
			$this->modulo2->actualizarModulo($pottot, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo);
		}
                
                //Guarda módulo III
                if ($data["modulo3"]["op3"]=='insert'){
			//Insertar el registro en la tabla del modulo III
			$this->modulo3->insertarModulo($intio, $inalo, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo);
		}
		elseif($data["modulo3"]["op3"]=='update'){
			//Update sobre el registro de la tabla del modulo III
			$this->modulo3->actualizarModulo($intio, $inalo, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo);
		}
                
                //Guarda módulo IV
                if ($data["modulo4"]["op4"]=='insert'){
			//Insertar el registro en la tabla del modulo IV
			$this->modulo4->insertarModulo($ihdo, $icda, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo);
		}
		elseif($data["modulo4"]["op4"]=='update'){
			//Update sobre el registro de la tabla del modulo IV
			$this->modulo4->actualizarModulo($ihdo, $icda, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo);
		}
                $index="miniencuesta";
                $fecha =  date("Y-m-d H:i:s");
                $data['envioform'] = $this->envioform->obtenerModulo($nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo);
                //Guarda la observación
                if ($data['envioform']['opobs']=='insert'){
                    //Insertar el registro en la tabla observaciones
		...
		elseif($data['envioform']['opobs']=='update'){
                    //Update sobre el registro de la tabla observaciones
		...
		$tipo_usuario = $this->session->userdata("tipo_usuario");
                ...
                if($tipo_usuario==9){  
                    $this->control->actualizarNovedadEstado($nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo, 99, 4); //Actualizar el estado de control por parte de la fuente
                }
                elseif($tipo_usuario==2){ //Guarda las observaciones de la critica
                    $this->control->actualizarNovedadEstado($nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo, 99, 5); //Actualizar el estado de control por parte del critico
                    $data['observaciones'] = $this->observacion->consultarObservacion($index, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo);
                    $fecha = date("Y-m-d h:i:s");
                    if ($data['observaciones']['opobs']=='insert'){
                        $this->observacion->guardarObservacion(1, 'miniencuesta', $observacionesCritica, $fecha, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo, $idcritico);
                    }
                    elseif($data['observaciones']['opobs']=='update'){
                        $this->observacion->actualizarObservacion($observacionesCritica, $index, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo);
                    }
                }    
	...
	/**
	 * Guarda/Registra los datos de los nuevos estableciminetos.
	 * @author sjneirag
	 * @since  08/05/2017
	 */
	public function actualizarModulo(){
		...
		$data["estab"]=$this->modulo1->obtenerEstablecimiento($nro_orden, $idnomcomest);
		
		$cuenta=count($data["estab"]);
		if($cuenta >=1){
			echo "Ya existe un establecimineto con el nombre que intenta registrar";
		}else{
			$data["ultimo_estab"]=$this->modulo1->obtenerEstablecimientos();
			
			$nro_establecimiento=$data["ultimo_estab"]["ultimoEstab"]+1;
			
			$this->modulo1->insertModuloEst($nro_orden,$nro_establecimiento, $idnomcomest, $iddireccest, $idactivest, $iddeptoest, $idmpioest, $idtelnoest, $idfaxnoest, $idcorreoest, $nom_cadena, $nom_operador, $ano_periodo, $mes_periodo);
			//$this->control->insertarControl($nro_orden,$nro_establecimiento,$ano_periodo, $mes_periodo, $fk_sede, $fk_subsede);
			echo "Registro exitoso";
		}
	}
	
	/**
	 * Registra cieere de establecimientos.
	 * @author sjneirag
	 * @since  09/05/2017
	 */
	public function actualizarCierreEst(){
		$this->load->model("control");
		$this->load->model("modulo1");
		$ano_periodo = $this->session->userdata("ano_periodo");
		$mes_periodo = $this->session->userdata("mes_periodo");
		...
		$this->modulo1->actualizaCierreEst($nro_estab, $estado_establecimiento, $motivoCierre, $anio, $mes);
		echo "Registro exitoso";
                $estado = $this->control->obtenerEstadoModulo($nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo,'modulo2');
		if ($op=='insert'){
			//Insertar el registro en la tabla del modulo II
			$this->modulo2->insertarModulo($potpsfr, $potperm, $pottcde, $pottcag, $potpau, $pottot, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo);
			//Actualizar el registro de la tabla de control
			
		}
		elseif($op=='update'){
			//Update sobre el registro de la tabla del modulo II
			$this->modulo2->actualizarModulo($potpsfr, $potperm, $pottcde, $pottcag, $potpau, $pottot, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo);
			//Actualizar el registro de la tabla de control
	...
	public function actualizarModuloIII(){
		...
		if (($op=='insert')&&($estado==0)){
			//Insertar el registro en la tabla del modulo III
			$this->modulo3->insertarModulo($inalo, $inalovd, $inaloatc, $inali, $inba, $insr, $inoe, $inoio, $intio, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo);
			//Actualizar el registro de la tabla de control
			...
			//Update sobre el registro de la tabla del modulo III
			$this->modulo3->actualizarModulo($inalo, $inalovd, $inaloatc, $inali, $inba, $insr, $inoe, $inoio, $intio, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo);
			//Actualizar el registro de la tabla de control
			$this->observaciones->actualizarObservacion($id_usuario, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo, 3, 'intio1', $intio, $obsintio1);
			$this->observaciones->actualizarObservacion($id_usuario, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo, 3, 'intio2', $intio, $obsintio2);
			$this->observaciones->actualizarObservacion($id_usuario, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo, 3, 'inalo', $inalo, $obsinalo);
	...
	public function actualizarModuloIV(){
		...
		if (($op=='insert')&&($estado==0)){
			//Insertar el registro en la tabla del modulo IV
			$this->modulo4->insertarModulo($ihdo, $ihoa, $ihoavd, $ihoatc, $icda, $icva, $ihpn, $ihpnr, $huetot, $mvnr, $mvcr, $mvor, $mvsr, $mvotr, $mvam, $mvamnr, $mvott, $mvnnr, $mvcnr, $mvonr, $mvsnr, $mvotnr, $mvottnr, $thsen, $thusen, $thdob, $thudob, $thsui, $thusui, $thmult, $thumult, $thotr, $thuotr, $thtot,  $tphto, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo);
			...
			$this->observaciones->actualizarObservacion($id_usuario, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo, 4, 'mvam', $mvam, $obsmvam);
			$this->observaciones->actualizarObservacion($id_usuario, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo, 4, 'mvamnr', $mvamnr, $obsmvamnr);
			...
			//Update sobre el registro de la tabla del modulo IV
			  $this->modulo4->actualizarModulo($ihdo, $ihoa, $ihoavd, $ihoatc, $icda, $icva, $ihpn, $ihpnr, $huetot, $mvnr, $mvcr, $mvor, $mvsr, $mvotr, $mvam, $mvamnr, $mvott, $mvnnr, $mvcnr, $mvonr, $mvsnr, $mvotnr, $mvottnr, $thsen, $thusen, $thdob, $thudob, $thsui, $thusui, $thmult, $thumult, $thotr, $thuotr, $thtot,  $tphto, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo);
			//Actualizar el registro en la tabla de control
			$this->observaciones->actualizarObservacion($id_usuario, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo, 4, 'ihoa', $ihoa, $obsihoa);
			$this->observaciones->actualizarObservacion($id_usuario, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo, 4, 'icva', $icva, $obsicva);
			$this->observaciones->actualizarObservacion($id_usuario, $nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo, 4, 'huetot', $huetot, $obshuetot);
	...
	public function obtenerObservaciones(){
		...
		$idusuario = $this->session->userdata("id");
		$nro_orden = $this->session->userdata("nro_orden");	
		$nro_establecimiento=$this->session->userdata("unidad_local");
