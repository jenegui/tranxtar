...
class Modulo1 extends CI_Model {
    ...
    function obtenerModuloOLD($nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo){
    	$modulo1 = array();
    	$sql = "SELECT CTRL.modulo1, CTRL.nro_orden, EMP.cambio_nit, EMP.idnit, EMP.iddv, EMP.idnit_nuevo, EMP.iddv_nuevo, EMP.idproraz, EST.idnomcom, EMP.idsigla, EMP.iddirecc, EMP.fk_depto, 
    	...
    	if ($query->num_rows() > 0){
    		foreach ($query->result() as $row){
    			...
    			$modulo1["cambio_nit"] = $row->cambio_nit;
                        $modulo1["idnit"] = $row->idnit;
                        $modulo1["iddv"] = $row->iddv;
                        $modulo1["idnit_nuevo"] = $row->idnit_nuevo;
                        $modulo1["iddv_nuevo"] = $row->iddv_nuevo;
    ...
    function obtenerModulo($nro_orden, $nro_establecimiento, $ano_periodo, $mes_periodo){
    	$modulo1 = array();
    	$sql = "SELECT CTRL.modulo1, CTRL.nro_orden, CTRL.nro_establecimiento,EMP.cambio_nit, EMP.idnit, EMP.iddv, EMP.idnit_nuevo, EMP.iddv_nuevo, EMP.idproraz, EST.idnomcom, EMP.idsigla, EMP.iddirecc, EMP.fk_depto, 
                EMP.fk_mpio, EMP.idtelno, EMP.idfaxno, EMP.idpagweb, EMP.idcorreo, DATE_FORMAT(EST.finicial,'%d/%m/%Y') AS finicial, DATE_FORMAT(EST.ffinal,'%d/%m/%Y') AS ffinal, EST.idnomcom AS idnomcomest, EST.idsigla AS idsiglaest, 
                EST.iddirecc AS iddireccest, EST.fk_depto AS iddeptoest, EST.fk_mpio AS idmpioest, EST.idtelno AS idtelnoest, EST.idfaxno AS idfaxnoest, EST.idcorreo AS idcorreoest, EST.nom_cadena AS nom_cadena, EST.nom_operador AS nom_operador, EST.fk_ciiu AS fk_ciiu,
                CIU.nom_ciiu AS nom_ciiu
                FROM rmmh_admin_control CTRL, rmmh_admin_empresas EMP, rmmh_admin_establecimientos EST, rmmh_param_ciiu3 CIU
                WHERE CTRL.nro_orden = EMP.nro_orden
    	...
    	if ($query->num_rows() > 0){
            foreach ($query->result() as $row){
                $modulo1["registros"] = $query->num_rows();
                $modulo1["nro_orden"] = $row->nro_orden;
                ...
                $modulo1["idnit_nuevo"] = $row->idnit_nuevo;
                $modulo1["iddv_nuevo"] = $row->iddv_nuevo;
                $modulo1["idproraz"] = utf8_decode(strtoupper($row->idproraz));
                $modulo1["idnomcom"] = utf8_decode(strtoupper($row->idnomcom));
                $modulo1["idsigla"] = strtoupper($row->idsigla);
                $modulo1["iddirecc"] = strtoupper($row->iddirecc);
                $modulo1["iddepto"] = $row->fk_depto;
                $modulo1["idmpio"] = $row->fk_mpio;
    ...
    function actualizarModulo($nro_orden, $nro_establecimiento, $cambionit, $idnit, $iddv, $idnitnuevo, $iddvnuevo, $idproraz, $iddirecc, $iddepto, $idmpio, $idtelno, $idfaxno, $idpagweb, $idcorreo, $finicial, $ffinal){
    	...
    	$data = array('cambio_nit' => $cambionit,
            'idnit' => $idnit, 
            'iddv' => $iddv,  
            'idnit_nuevo' => $idnitnuevo, 
            'iddv_nuevo' => $iddvnuevo,
            'idproraz' => $idproraz, 
            'iddirecc' => $iddirecc, 
            'idtelno' => $idtelno, 
            'idfaxno' => $idfaxno, 
