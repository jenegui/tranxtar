...
class Periodo extends CI_Model {
	...
	//Obtiene todos los periodos que se han creado en la B.D. (Se concatena el a�o y el mes).
	...
	//Obtiene el nombre de un periodo (A�o y Mes) - Se utiliza en el header del modulo.
	...
	//Obtiene el periodo m�s reciente que est� creado en la base de datos
    ...
    function crearNuevoPeriodo($ano_periodo, $mes_periodo){
    	...
    	// NOTA: Si la novedad se encuentra en estado 3, 4 � 41, la novedad pasa al siguiente periodo como 
     ...
     * @since  Junio 13 / 2013
     * Se crea una nueva funci�n para el cierre de periodos. Se modifica el cierre de periodo junto con las modificaciones realizadas
     * al aplicativo para realizar el cierre de acceso a las fuentes a periodos anteriores. Se deja constancia que la modificaci�n se 
    ...
    function crearNuevoPeriodoMODIFICADO($ano_periodo, $mes_periodo){
		...
		$data = array('ano_periodo' => $ano_nuevo, 
		              ...
		              'estado' => 'A');
		$this->db->insert('rmmh_param_periodos', $data);
		//Ya que fue creado el nuevo periodo, obtengo el periodo (m�s reciente).
		$periodo = $this->obtenerPeriodoActual(); //Obtener el ultimo periodo y mas reciente.
		//Duplicar todas las fuentes para el periodo actual
		$sql = "INSERT INTO rmmh_admin_control (nro_orden, nro_establecimiento, ano_periodo, mes_periodo, nueva, modulo1, modulo2, modulo3, modulo4, modulo5, envio, inclusion, control, fk_novedad, fk_estado, fk_sede, fk_subsede, fk_usuariocritica, fk_usuariologistica)
		        SELECT C.nro_orden, C.nro_establecimiento, ".$ano_nuevo.",".$mes_nuevo.", 0, 0, 0, 0, 0, 0, 0, C.inclusion, 'A',
		...
		$query = $this->db->query($sql);       
		//echo "MMM".$sql;
		//NOTA: Seg�n el nuevo requerimiento, las novedades no pasan entre periodos. Por lo que las novedades no son transferidas entre periodos. 
    ...
    //Actualizar salario 
    function actualizarSalarioPeriodo($salario, $ano_periodo, $mes_periodo){
    	$data = array('smmlv_periodo' => $salario);
        $this->db->where("ano_periodo", $ano_periodo);
        $this->db->where("mes_periodo", $mes_periodo);
        $this->db->update('rmmh_param_periodos', $data);
        $this->db->close();
